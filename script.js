document.addEventListener("DOMContentLoaded", () => {
    ...
    let currentTopic = "javascript" // Track the current topic (javascript, html, css)
    ...
    // JavaScript Challenges with difficulty-based time limits
    ...
    function createDoors() {
      ...
      // Create the three main topic buttons
      ...
      topics.forEach((topic, index) => {
        ...
        door.addEventListener("click", () => {
          ...
          startTime = Date.now() // Track start time for the entire game
  
          // Open the first question of the selected topic
              ...
              setTimeout(() => {
                ...
                if (currentQuestionIndex < 5) {
                  ...
                  if (currentDoor < 10) {
                    ...
                    doorTimes.push({ door: currentDoor, topic: currentTopic, time: doorTime }) // Store door time
                    currentDoor++
                    currentQuestionIndex = 0
                    doorStartTime = Date.now() // Reset start time for the next door
  
                    // Open the first question of the next door
    ...
    function cleanupEventListeners() {
      const newSubmitButton = submitCodeButton.cloneNode(true)
      submitCodeButton.parentNode.replaceChild(newSubmitButton, submitCodeButton)
    ...
    function handleSubmit() {
      // This will be handled in the specific challenge type handlers
    }
  
    function showHint() {
      hintContainer.style.display = "block"
      hintText.textContent = allChallenges[currentTopic][(currentDoor - 1) * 5 + currentQuestionIndex].hint
    }
